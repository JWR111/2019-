...
刪'L1YR_GROSS_PRE_AMT','X_G_IND_Y','X_H_IND_Y','IF_ISSUE_INSD_B_IND_N','IF_ISSUE_INSD_F_IND_N','IF_ISSUE_INSD_G_IND_N','IF_ISSUE_INSD_K_IND_N','CUST_9_SEGMENTS_CD_E'
...
| 初始分數 | 最終分數|
| -------- | -------- | 
| 0.7623384095    | 0.7615579349     | 
2. 缺失值處理後,並經調整參數後預測結果

| 初始分數 | 最終分數|
| -------- | -------- | 
| 0.8411716007    | 0.8428757536     |
3. 而後新增變數預測結果

| 初始分數 | 最終分數|
| -------- | -------- | 
| 0.8390084873    | 0.838226414     |
- xgboost
1. 缺失值處理後,並經調整參數後預測結果

| 初始分數 | 最終分數|
| -------- | -------- | 
| 0.846484467    | 0.8449674382     |
2. 而後新增變數預測結果

| 初始分數 | 最終分數|
| -------- | -------- | 
| 0.8451270795    | 0.8432421421     |
- stacking
1. 缺失值處理後,並經調整參數後預測結果

| 初始分數 | 最終分數|
| -------- | -------- | 
| 0.8445705042	    | 0.8461577319	|
2. 而後新增變數預測結果

| 初始分數 | 最終分數|
| -------- | -------- | 
| 0.8406240677	    | 0.8399462692	|

- 從以上結果可以得知新增變數造成模型效果更差,可能的原因為過度擬合,此時有考慮利用重要性選取變數測試,但由於時間緊迫,最後沒有繼續測試
- 由以上預測結果也可看出stacking將各個模型的預測結果整合起來確實效果會比僅使用一個模型的預測效果來得好,較不易有過度擬合的情況
- 最終AUC分數為0.8461577319,前五名分數分別為0.854856,0.852604,0.849963,0.849341,0.849252
